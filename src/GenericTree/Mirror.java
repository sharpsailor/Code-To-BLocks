...
//		  }
	  ...
	  public static void linearTraversal(Node node) {
		  
		  for(Node child:node.children) {
			 linearTraversal(child);
		  }
		  while(node.children.size()>1) {
			  Node lastNode = node.children.remove(node.children.size()-1);
			  Node SecondLast= getTail(node.children.get(node.children.size()-1));
			  SecondLast.children.add(lastNode);
		  }
